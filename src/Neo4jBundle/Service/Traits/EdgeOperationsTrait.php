...
trait EdgeOperationsTrait
{
    public function createEdge(array $fromNode, array $toNode, string $type, array $properties = []): array
    {
        ...
        return ['id' => $record->get('id'), 'type' => $record->get('type'), 'properties' => $record->get('props')];
    }
    ...
    public function getNeighbors(array $node, string $relationshipType = null): array
    {
        ...
        $query = "MATCH (n)-$relTypeQuery-(neighbor) WHERE id(n) = $id RETURN id(neighbor) AS id, properties(neighbor) AS props";
        $result = $this->client->run($query, ['id' => (int)$node['id'], 'relationshipType' => $relationshipType]);

        return array_map(fn($record) => ['id' => $record->get('id'), 'properties' => $record->get('props')], $result->getRecords());
    }
}
